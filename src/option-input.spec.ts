...
describe('ZOptionInput', () => {
  describe('valuesOf', () => {
    it('extracts all positional options up to the next named one', () => {
      expect(ZOptionInput.valuesOf(['1', '2', '-', '3', '--named', 'next'])).toEqual([
        '1',
        '2',
        '-',
        '3',
    ...
    it('extracts all positional options up to the next `--`', () => {
      expect(ZOptionInput.valuesOf(['1', '2', '-', '3', '--', 'last'])).toEqual([
        '1',
        '2',
        '-',
        '3',
  ...
  describe('equal', () => {
    ...
    it('compares values', () => {
      expect(ZOptionInput.equal({ name: 'a', values: ['a'] }, { name: 'a', values: ['a'] })).toBe(
        true,
      );
      expect(ZOptionInput.equal({ name: 'a' }, { name: 'a', values: [] })).toBe(true);
      expect(ZOptionInput.equal({ name: 'a', values: [] }, { name: 'a' })).toBe(true);
      expect(ZOptionInput.equal({ name: 'a', values: ['a'] }, { name: 'a', values: ['b'] })).toBe(
        false,
      );
      expect(
        ZOptionInput.equal({ name: 'a', values: ['a'] }, { name: 'a', values: ['a', 'b'] }),
      ).toBe(false);
    });
    it('compares tails', () => {
      ...
      expect(ZOptionInput.equal({ name: 'a', tail: ['a'] }, { name: 'a', tail: ['b'] })).toBe(
        false,
      );
      expect(ZOptionInput.equal({ name: 'a', tail: ['a'] }, { name: 'a', tail: ['a', 'b'] })).toBe(
        false,
      );
    });
    it('compares retries', () => {
